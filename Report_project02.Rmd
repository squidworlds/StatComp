...
colnames(comparison_df) <- c("$\\text{Model}$", "$R^2$", "$\\text{Adjusted } R^2$", "$\\text{AIC}$", "$\\text{BIC}$")
```
...
best_scores <- monthly_rolling_model(demand_no_outliers, best_formula)
best_scores$model <- "best"

all_scores <- rbind(m0_scores, best_scores)

knit_print.all_scores <- lemon_print
```
...
```{r reg_plot, warning = FALSE, echo=TRUE, out.width="50%", cache = FALSE}
reg_plot
```
...
```{r scores_plot, warning = FALSE, echo = TRUE, out.width="50%", cache = FALSE}
