...
hourly_temp <- read.csv("C:\\Users\\Ella Park\\Desktop\\Year 3\\Sem 2\\Statistical Computing\\Project02\\Project02\\SCS_hourly_temp.csv")
...
m_both <- formula(step(lm(m_0_year_te, demand), direction = "forward", trace = 0))
...
m_final <- demand_gross ~ wind + TE + day + month + poly(year, 3) + TE:month


## forward and backward?

# putting all these models into a vector for comparison
formulas <- list(m_0, m_0_start_year_cubed, m_0_year_te_daytype, m_prestep, m_both, m_0_year_te, m_final)
names <- c("m0", "Poly start_year, TE, interactions", "Poly start_year, TE, interactions, poly daytype", "Prestep", "Backward and Forward", "No interactions", "Final")
...
final_prediction <- lm_predicting(m_final, demand)
...
First, it is important to note that our data now has $daytype$ and $month$ as factor variables. We will perform rolling-window cross-validation to compare how the model predicts in the different winter months that we are given. We will train on 3 years, and test on the next year, then move our testing window forward by 1 year. Here I am comparing the basic model, $m0$, and our final model, $Best$?.
...
final_scores <- monthly_rolling_model(demand, m_final)
final_scores$model <- "Final"

all_scores <- rbind(m0_scores, final_scores)
...
colnames(score_summary) <- c("Model", "Month", "Mean SE", "Mean DS", "Mean Interval Score")

```

```{r scores_table, warning = FALSE, echo=TRUE, out.width="50%", cache = FALSE, caption="Score comparison of models with respect to month."}
kable(score_summary)
```
...
reg_plot_month <- ggplot(all_scores, aes(x = actual, y = mean, color = model)) +
...
reg_plot_daytype <- ggplot(all_scores, aes(x = actual, y = mean, color = model)) +
  geom_point(alpha = 0.4) +                      # Scatter plot of actual vs predicted
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black") +  # Line y=x for reference
  facet_wrap(~daytype) +
  #ylim(2*10^4, 8*10^4) +
...
```{r reg_plot_daytype, warning = FALSE, echo=TRUE, cache = FALSE, out.width="100%", caption = "Model prediction comparison with respect to month."}
reg_plot_daytype
```

These plots show that our model ? is significantly better than $m0$ for predicting on weekdays, but the difference between the two models is not as clear for weekend data. However, this could be because there is less weekend data, so to further analyse, we look more closely at scores.
...
We can also look at scores for each month in the following figure.
...
p_values <- statistic %>%
  summarise(se = mean(abs(se) > abs(statistic0$se)),
            ds = mean(abs(ds) > abs(statistic0$ds)))
