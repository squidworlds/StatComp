...
pp <- ggplot(m0_data, aes(x = demand_gross, y = mean_pred, color = daytype)) +
  ...
  geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = daytype), alpha = 0.2) +
  # Prediction interval ribbon
  geom_ribbon(aes(ymin = lower_pi, ymax = upper_pi, fill = daytype), alpha = 0.3) +
  # Line for mean predictions
  geom_line(size = 1) +
  # Add points for actual data
  geom_point(aes(y = demand_gross), alpha = 0.5, shape = 1)
```
