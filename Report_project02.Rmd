...
m_0 <- lm(demand_gross ~ wind + solar_S + temp + factor(wdayindex) + month, data = demand) 

m_0_start_year_cubed <- lm(demand_gross ~ wind + solar_S + temp + factor(wdayindex) + month + poly(start_year, 3), data = demand)

m_0_year_te <- lm(demand_gross ~ wind + solar_S + TE + factor(wdayindex) + month + poly(start_year, 3), data = demand)

m_0_year_te_daytype <- lm(demand_gross ~ wind + solar_S + TE + daytype + month + poly(start_year, 3), data = demand)

m_prestep <- lm(demand_gross ~ (wind + solar_S + TE + factor(wdayindex) + month + poly(start_year, 3))^2, data = demand)

m_final <- step(m_prestep, direction = "backward")

# putting all these models into a vector for comparison
lms <- list(m_0, m_0_start_year_cubed, m_0_year_te_daytype, m_prestep, m_final)
names <- c("m0", "Poly start_year, TE, interactions", "Poly start_year, TE, interactions, poly daytype", "Best model")
...
%>% 
  mutate(
    se_diff = `se_m0` - `se_Best`,  # Use backticks to reference column names
    ds_diff = `ds_m0` - `ds_Best`   # Use backticks to reference column names
  ) %>%
