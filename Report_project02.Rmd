...
Accurately forecasting future daily peak electricity demand is crucial for NESO's (National Electricity System Operator) planning and ensuring security of supply, especially during winter when cold weather drives up demand and the risk of shortfalls is highest. To help anticipate extreme scenarios, we focus on modelling the upper tail of the demand distribution to help NESO allocate reserves efficiently.
...
This report develops and refines a linear regression model to estimate future peak demand. Starting with a baseline model $M_0$ using key predictors such as wind and solar generation, temperature, and time-related factors, we test refinements by adding covariates, accounting for long-term trends, and exploring alternative temperature metrics. In order to compare and select our best model, we evaluate how well our model fits historic data, predictive accuracy and how peak demand varies under different weather conditions. To compare prediction accuracy, we construct a rolling-window cross-validation scheme that computes prediction scores across months and then weekdays versus weekends.

The final model is selected for its predictive performance and interpretability, providing NESO with a reliable tool for demand forecasting. We also address the model's limitations and suggest further improvements to enhance its accuracy and robustness. Our exploratory analysis, model development and testing is a simple model that could be improved by incorporating more recent data, environmental factors and covariate-dependence.

# Data description and exploratory analysis

A thorough understanding of the dataset is essential for building an effective forecasting model. This section explores key features relevant to modelling, highlighting important patterns through visualizations to inform model development and summarizes the pre-analysis steps undertaken.

During the preprocessing phase, we confirmed that there were no missing values (NA) in the dataset. Outliers were examined, particularly those that could have been incorrectly inputted, ensuring data quality for model fitting. If we had evidence to support this for any data point we could have excluded it, however we didn't. This was also confirmed when checking the Cook's distance on leverage plots of our final model. The nature of each variable was checked to ensure correct data types (e.g. continuous, categorical) and categorical variables were renamed for easier analysis.

We focus by analyzing the continuous variables and their relationship to peak electricity demand. The key predictors to explore are:
...
Most points fall below 0.05, showing little correlation between solar and demand. However, a clear negative correlation appears beyond this threshold, suggesting that higher solar generation reduces peak demand. This is intuitive, as increased solar output reduces electricity needs for lighting supporting the inclusion of solar as an explanatory variable in our model.
...
Friday stands out with slightly lower peak demand compared to other weekdays with its median nearly 2,000 MW lower than the weekdays likely due to reduced business activity as the weekend approaches. Despite some variation, the medians for the weekdays are close, indicating relatively consistent demand throughout the workweek. Weekdays have a higher median peak demand (around 52,000 MW), while weekends show lower values (around 45,000 MW). 

Including day of the week as a factor variable in the model ensures that variations across different days are incorporated into the model. This helps the model reflect real demand patterns without oversimplification, and later analysis confirms its usefulness and statistical significance.
...
Furthermore, the initial model did not take capture long-term trends in electricity demand. The previous exploratory visualizations revealed that year variable, `year`, influenced peak demand, particularly in a cubic pattern. This insight led to the inclusion of `poly(year, 3)`. This was selected instead over the non-orthogonal polynomial, `year + I(year^2) + I(year^3)` since poly generates orthogonal polynomials ensuring the terms are uncorrelated to reduce multicollinearity.
...
These plots show that the majority of scores for each value of $demand\_gross$ using our model is better than those for $m0$. Additionally, the scores for higher gross demand are lower, which means that our model is better at predicting extreme values.

## Exchangeability Test

We want to investigate whether our final model is better at predicting than $m0$. If they are equivalent, it should not matter, on average, if we randomly swap their prediction scores (SE and DS) within each cross-validation score pair $(S_i^A, S_i^B)$. We use the test statistic $\frac{1}{N} \sum_{i=1}^n(S_i^A - S_i^B)$, and make a Monte Carlo estimate of the p-value for a test of exchangeability between model predictions from $m0$ and from our model against the alternative hypothesis that our model is better than $m0$.
...
Temperature is a key driver of electricity demand, especially during winter, when colder conditions increase heating usage. NESO currently use TE to account for temperature which is the rolling average of the previous day's TO (average temperature between 3pm and 6pm). To assess whether TE is the most effective way to capture the temperature-demand relationship, we first examined the explanatory power of using TO directly in the model.

Using TO as the temperature variable in our final model had a lower adjusted $R^2$ suggesting that daily temperature alone does not fully account for the variation in demand since using TE had an adjusted $R^2$ of 77.24%. This is likely because people’s energy use responds to patterns of cold weather over several days, not just the temperature on a single day.

We also experimented calculating TO over both a larger and shorter window for the TE variable, which resulted in lower $R^2$ values compared to using our current TE variable. Additionally, we explored rolling averages over 2, 3, and 5 days to. As the window size increased, the adjusted $R^2$ decreased suggesting that shorter windows capture more relevant variation in demand.
